            ...
            if 'conv' in name:
                ...
                for i in range(shape[0]):
                    for j in range(shape[1]):
                        sub_mat = csr_matrix(weight[i,j,:,:]) if shape[0] < shape[1] else csc_matrix(weight[i,j,:,:])
                        sub_data += sub_mat.data.tolist()
                        indices += sub_mat.indices.tolist()
                        indptrs += calc_index_diff(sub_mat.indptr).tolist()
                        data_len.append(len(sub_data))
                        
                sub_data = np.array(sub_data, dtype=np.float32)
                indices = np.array(indices, dtype=np.int32)
                indptrs = np.array(indptrs, dtype=np.int32)
                data_len = np.array(data_len, dtype=np.int32)
                
                # Encode
                t0, d0 = huffman_encode(sub_data, name+f'_{form}_data', directory)
                t1, d1 = huffman_encode(indices, name+f'_{form}_indices', directory)
                t2, d2 = huffman_encode(indptrs, name+f'_{form}_indptr', directory)
                t3, d3 = huffman_encode(data_len, name+f'_{form}_data_length', directory)
    ...
    for name, param in model.named_parameters():
        if 'weight' in name:
            ...
            if 'conv' in name:
                ...
                data_len = huffman_decode(directory, name+f'_{form}_data_length', dtype='int32')
                
                # Construct matrix
                first_channel = []
                for i in range(shape[0]):
                    first_channel += [i] * shape[1]
                second_channel = list(range(shape[1])) * shape[0]
                channel_ind = 0
                
                cnt = 0
                pre = 0
                length = data_len.shape[0]
                indptr_len = shape[3]
                for i in range(length):
                    sub_data = data[pre:data_len[i,],]
                    sub_indices = indices[pre:data_len[i,],]
                    sub_indptr = reconstruct_indptr(indptr[indptr_len*cnt:indptr_len*(cnt+1),])
                    sub_mat = matrix((sub_data, sub_indices, sub_indptr), shape[2:])
                    conv[first_channel[channel_ind],second_channel[channel_ind],:,:] = sub_mat.toarray()
                    pre = data_len[i,]
                    channel_ind += 1
                    cnt += 1
