...
def apply_weight_sharing(model, bits=5):
    ...
    for name, module in model.named_children():
        ...
        if len(shape) == 2:  # fully connected layers
            ...
            unpruned_indices = np.where(weight!=0)
            unpruned_weights = weight[unpruned_indices]

            # weight sharing by kmeans
            space = np.linspace(min(unpruned_weights), max(unpruned_weights), num=quan_range)
            kmeans = KMeans(n_clusters=len(space), init=space.reshape(-1, 1), n_init=1, precompute_distances=True, algorithm="full")
            kmeans.fit(unpruned_weights.reshape(-1, 1))
            new_weight = kmeans.cluster_centers_[kmeans.labels_].reshape(-1)
            weight[unpruned_indices] = new_weight
